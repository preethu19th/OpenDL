...
public final class ModelReplicaSplit<T> implements Serializable {
     ...
     * @return
     */
    public JavaPairRDD<Integer, Iterable<T>> split(JavaRDD<T> input, int nrModelReplica, SGDTrainConfig config) {
        JavaPairRDD<Integer, Iterable<T>> output = input
                .mapToPair(new SplitModelReplica(nrModelReplica))
                .groupByKey().persist(config.getMrDataStorage());
        output.count();
        return output;
    }
